    ...
    const fetchVersions = async () => {
      try {
        ...
        const res = await fetch(
          `/api/version-control/list?projectId=${projectId}`
        );
          ...
          (data.versions || []).map(async (v: VersionItem) => {
            try {
              const fileRes = await fetch(
                `/api/files/get-file-name?fileId=${v.fileId}`
              );
          ...
          {versions.map((v) => (
            ...
            >
              ...
              <div className="mt-4 flex gap-6 text-sm">
                <span className="text-green-600">+{v.stats.added} added</span>
                <span className="text-red-600">-{v.stats.removed} removed</span>
